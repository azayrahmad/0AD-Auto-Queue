...
var g_EntityCommands =
{
	...
	"autoqueue-on": {
		"getInfo": function(entStates)
		{
			if (entStates.every(entState => !entState.isproducingunits || entState.autoqueue))
				return false;
			return {
				"tooltip": translate("Turn on auto-queue for selected buildings."),
	...
	"autoqueue-off": {
		"getInfo": function(entStates)
		{
			if (entStates.every(entState => !entState.isproducingunits || !entState.autoqueue))
				return false;
			return {
				"tooltip": translate("Turn off auto-queue for selected buildings."),
